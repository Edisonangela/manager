...
!function(a, b) {
    ...
    function l(a, b, c, d) {
        var f = k(b.width, b.height, c), g = f.height, h = b.width * d, i = b.height * d, j = c.font ? c.font: "sans-serif";
    ...
    if (document.getElementsByClassName || (document.getElementsByClassName = function(a) {
        ...
        if (b.evaluate)
            for (d = ".//*[contains(concat(' ', @class, ' '), ' " + a + " ')]", c = b.evaluate(d, b, null, 0, null);
        e = c.iterateNext();
        )f.push(e);
        else 
            for (c = b.getElementsByTagName("*"), d = new RegExp("(^|\\s)" + a + "(\\s|$)")
                , e = 0;
        e < c.length;
        e++)d.test(c[e].className) && f.push(c[e]);
        return f
    ...
    }), e.getContext)if (e.toDataURL("image/png").indexOf("data:image/png") < 0)
        d=!0;
    else 
    ...
    a.add_theme = function(b, c) {
        ...
        var d = h(u, b), e = [], f = [], i = [];
        for ("string" == typeof d.images ? f = g(d.images) : window.NodeList && d.images instanceof window.NodeList ? f = d.images : window.Node && d.images instanceof window.Node && (f = [d.images]), "string" == typeof d.bgnodes ? i = g(d.bgnodes) : window.NodeList && d.elements instanceof window.NodeList ? i = d.bgnodes : window.Node && d.bgnodes instanceof window.Node && (i = [d.bgnodes]), c=!0, n = 0, l = f.length; l > n; n++)
            e.push(f[n]);
        ...
        for (var k = new RegExp(d.domain + '/(.*?)"?\\)'), l = i.length, n = 0; l > n; n++) {
            var p = window.getComputedStyle(i[n], null).getPropertyValue("background-image"), q = p.match(k), r = i[n].getAttribute("data-background-src");
            if (q) {
                var s = o(q[1].split("/"), d);
